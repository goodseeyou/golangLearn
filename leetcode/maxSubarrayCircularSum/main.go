...
func maxSubarraySumCircular(A []int) int {
	...
	k2 = sum + k2
	return Max(k1, k2)
	//kRight := sum + Kadene(A[1:])
	//kLeft := sum + Kadene(A[:len(A)-1])
	//return Max(k1, kRight, kLeft)
}

func Negatify(arr []int) {
	for i := 0; i < len(arr); i++ {
		arr[i] = -arr[i]
	}
}
