...
func ladderLength(beginWord string, endWord string, wordList []string) int {
	if !isDiffLimit(endWord, wordList, 0) || !isDiffLimit(beginWord, wordList, 1) {
		return 0
	}
	...
	for i := range wordList {
		...
		for j := 0; j < len(wordList); j++ {
			distance[i][j] = diffCount(wordList[i], wordList[j])
			if distance[i][j] != 1 {
				distance[i][j] = len(wordList) + 1
			}
	...
	beginIndexs := make([]int, len(wordList))
	for i, w := range wordList {
		beginIndexs[i] = len(wordList) + 1
		v := diffCount(beginWord, w)
		if v <= 1 {
			beginIndexs[i] = v
		}
	}

	doneNodes := make([]int, len(wordList))
	doneNodes[endIndex] = 1
	startNodes := make([]int, len(wordList))
	startNodes[endIndex] = 1
	for {
		if !confirmNode(endIndex, distance, startNodes, doneNodes, wordList) {
			break
		}
	...
	minV := len(wordList) + 1
	for bi, beginValue := range beginIndexs {
		if beginValue > 1 {
			continue
		}
		if distance[endIndex][bi] <= len(wordList) {

			v := distance[endIndex][bi] + 1 + beginIndexs[bi]
			if v < minV {
				minV = v
				/*
					fmt.Println(wordList[endIndex], wordList[bi], distance[endIndex][bi])
...
//dijkstra
func confirmNode(endIndex int, distance [][]int, startNodes, doneNodes []int, wordList []string) bool {
	getNode := false
	minNode := -1
	minD := 1<<31 - 1
	minStartNode := -1
	for i := range wordList {
		for startNode, n := range startNodes {
			if n != 1 {
				continue
			}
			if distance[startNode][i] == 1 && doneNodes[i] == 0 {
				tmpMin := min(distance[endIndex][i], distance[endIndex][startNode]+distance[startNode][i])
				if tmpMin < minD {
					minD = tmpMin
					minNode = i
	...
	if getNode {
		distance[endIndex][minNode] = minD
		doneNodes[minNode] = 1
		startNodes[minNode] = 1
		/*fmt.Println("-----", minStartNode, i)
		  print2D(distance)*/

		//no use
		minNode = minStartNode
	}
...
func isDiffLimit(w string, words []string, limit int) bool {
	for _, ws := range words {
		if diffCount(w, ws) <= limit {
			return true
		}
	}
...
func print2D(nums [][]int) {
	for i := 0; i < len(nums); i++ {
		for j := 0; j < len(nums); j++ {
			fmt.Printf("%3d ", nums[i][j])
		}
		fmt.Println()
	}
}

func diffCount(w1, w2 string) int {
	if len(w1) != len(w2) {
		return 1<<31 - 1
	}
	if w1 == w2 {
